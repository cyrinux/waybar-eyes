...
.PHONY: build
build: main.go
	go build -trimpath -ldflags "-X github.com/cyrinux/waybar-eyes/main.Version=$(VERSION) -linkmode=external" -o $(BIN) main.go

.PHONY: debug
debug: main.go
	go build -gcflags='all=-N -l' -ldflags "-X github.com/cyrinux/waybar-eyes/main.Version=$(VERSION) -linkmode=external" -o $(BIN) main.go
	dlv exec ./waybar-eyes
...
dist: clean vendor build
	...
	cp -r LICENSE.md README.md example models waybar-eyes.service "$(TMP)/$(BIN)-$(VERSION)-linux64"
...
install:
	...
	install -Dm644 -t "$(SHARE_DIR)/waybar-eyes/models" models/haarcascade_frontalface_default.xml
	...
	install -Dm644 -t "$(SHARE_DIR)/doc/$(BIN)/example" example/style.css
